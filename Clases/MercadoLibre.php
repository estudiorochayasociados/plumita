...
class MercadoLibre
{
    ...
    */
    ...
    public function updatePriceStock($id, $price, $stock, $type)
    {
        ...
        if (!empty($result['variations'])) {
            foreach ($result['variations'] as $key => $var) {
                $data['variations'][$key] = ['id' => $var['id'], 'available_quantity' => (int)$stock, 'price' => (float)$price];
            }
        } else {
            $data["price"] = (float)$price;
            $data["available_quantity"] = (int)$stock;
        }

        $data = json_encode($data, JSON_UNESCAPED_UNICODE);

        $meli = json_decode($this->f->curl("PUT", "https://api.mercadolibre.com/items/$id?access_token=" . $_SESSION["access_token"], $data), true);
        $response = ["status" => false, "data" => "", "error" => "", "price" => $price];
    ...
    public function update($id, $title, $price, $description, $stock, $images, $type, $config)
    {
        ...
        ($config['cfg-price']) ? $data["price"] = round($price) : '';
    ...
    public function list($filter, $order, $limit, $toMeli = '')
    {
        $array = [];
        if (is_array($filter)) {
            $filterSql = "WHERE ";
        ...
        $cod_product = empty($toMeli) ? ", `productos`.`cod_producto`" : '';

        !empty($order) ? $orderSql = $order : $orderSql = "id DESC";

        !empty($limit) ? $limitSql = "LIMIT " . $limit : $limitSql = '';

        $sql = "SELECT `mercadolibre`.* $cod_product  FROM `mercadolibre` LEFT JOIN `productos` ON `mercadolibre`.`product` = `productos`.`cod` $filterSql ORDER BY $orderSql $limitSql";
    ...
    {
        $sql = "SELECT * FROM `mercadolibre` WHERE  `product`= '$product' ";
        $check = $this->con->sqlReturn($sql);
