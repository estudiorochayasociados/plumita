...
$distribuidor = new Clases\Distribuidores();

$ciudad = $funciones->antihack_mysqli(isset($_POST['ciudad']) ? $_POST['ciudad'] : '');

$ciudadData = $distribuidor->listForCiudad($ciudad);
if (!empty($ciudadData)) {
    $response = '';
    foreach ($ciudadData as $ciudadData_) {
        ...
        $response .= "<h4 class=' label label-danger fs-14'>" . $ciudadData_['data']['distribuidor'] . "</h4>";
