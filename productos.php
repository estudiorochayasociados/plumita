...
if (!empty($categoria_get)) {
    foreach ($categoriasData as $cats) {
        $title = $funciones->normalizar_link($cats['data']['titulo']);
        if ($title == $categoria_get) {
            ...
            if (!empty($categoria_data_filtro)) $filter[] = "categoria='" . $categoria_data_filtro[0]['data']['cod'] . "'";
            break;
        }
    }
}
                            ...
                            <div class="col-md-12 mb-10">
                                <form class="login_form row" method="get" id="buscar">
                                    <div class="col-md-9 form-group">
                                        <input class="form-control" value="<?= !empty($titulo) ? $titulo : ''; ?>" type="text" placeholder="Buscar un producto" name="titulo" required>
                                    </div>
                            ...
                            <?php foreach ($productos_data as $prod) { ?>
                                <div class="col-md-4 col-sm-6">
                                    <div class="l_product_item">
                                        <div class="l_p_img" style="height:300px;background:url(<?= URL . '/' . $prod['images'][0]['ruta']; ?>) no-repeat center center/70%;">
                                        </div>
                                        <div class="l_p_text">
                                            <ul>
                                                <li>
                                                    ...
                                                    <?php if ($prod['data']['stock'] > 0 && $prod['data']['precio'] > 0) { ?>
                                                        <a class="add_cart_btn whiteColor" onclick="addToCartQuick('<?= $prod['data']['cod'] ?>',1,'<?= URL ?>')">Agregar</a>
                                                    <?php } else { ?>
                                                        <div class="add_cart_btn sinStockColorBtn">Sin stock</div>
                                                    <?php  } ?>
                                                </li>
                                            </ul>
                                            <h4><?= ucfirst(substr(strip_tags($prod['data']['titulo']), 0, 40)); ?></h4>
                                            <?php
                                            if ($prod['data']['precio'] > 0 && $prod['data']['stock'] > 0) {
                                            ?>
                                                <div style="margin-top:5px">
                                                    <span class="fs-20"><?= $prod['data']['precio_descuento'] > 0 ? "$" . $prod['data']['precio_descuento'] : "$" . $prod['data']['precio'] ?></span>
                                                    <strike class="fs-14"><?= $prod['data']['precio_descuento'] > 0 ? "$" . $prod['data']['precio'] : '' ?></strike>
                                                </div>
                                            <?php
                                            } ?>
                                        </div>
                                    ...
                                    <div class="pagination mb-60">
                                        <ul class="pagination text-center">
                                            ...
                                            <?php for ($i = 1; $i <= $numeroPaginas; $i++) { ?>
                                                <li><a href="<?= $url ?><?= $anidador ?>pagina=<?= $i ?>" class="<?php if ($i == $pagina + 1) echo "active"; ?>"><?= $i ?></a></li>
                                            <?php } ?>
                        ...
                        <aside class="l_widgest l_p_categories_widget">
                            ...
                            <ul class="navbar-nav">
                                <?php foreach ($categoriasData as $cats) { ?>
                                    <li class="nav-item">
                                        <a class="nav-link" href="<?= URL . '/productos/c/' . $funciones->normalizar_link($cats['data']['titulo']); ?>">
                                            <?= ucfirst(substr(strip_tags($cats['data']['titulo']), 0, 60)); ?>
                                        </a>
                                    </li>
                                <?php } ?>
                            </ul>
                            ...
                            <?php foreach ($productos_data_random as $proRand) { ?>
                                <div class="media">
                                    <a href="<?= URL . '/producto/' . $funciones->normalizar_link($proRand['data']["titulo"]) . '/' . $proRand['data']['cod'] ?>">
                                        <div class="d-flex" style="height:100px;width:80px;background:url(<?= URL . '/' . $proRand['images'][0]['ruta']; ?>) no-repeat center center/contain;">
                                        </div>
                                    </a>
