                ...
                if (isset($_POST['submit'])) {
                    if (isset($_FILES['excel']['name']) && $_FILES['excel']['name'] != "") {
                        ...
                        foreach ($images as $images_) {
                            $imagenesArray[$images_->getCoordinates()] = [
                                "path" => $images_->getPath(),
                                "extension" => $images_->getExtension()
                            ];
                        }
                        ...
                        foreach ($sheet as $key => $cellVal) {
                            ...
                            $cod_producto = $funciones->antihack_mysqli($cellVal[13]);
                            ...
                            if (!empty($imagenesArray[$coor])) {

                                $imagenes->set("cod", $cod);
                                $imagenes->deleteAll();

                                $nombre_imagen = substr(md5(uniqid(rand())), 0, 11);

                                $image = $imagenesArray[$coor]['path'];
                                ...
                                fclose($zipReader);
                                $extension = $imagenesArray[$coor]['extension'];

                                $myFileName = "../assets/archivos/recortadas/" . $nombre_imagen . '.' . $extension;
